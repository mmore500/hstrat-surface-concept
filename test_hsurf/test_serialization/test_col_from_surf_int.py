...
def test_col_from_surf_packet_bytes(
    ...
    if sentry:
        packet_bit_length = len(packet) * 8
        packet_value |= 1 << packet_bit_length

    deserialized_column = hsurf.col_from_surf_int(
        value=packet_value,
        differentia_bit_width=8,
        site_selection_algo=site_selection_algo,
        value_byte_width=len(packet) if not sentry else None,
    )
    ...
    assert (
        column._num_strata_ingested
        == deserialized_column._num_strata_ingested
    )
    assert (
