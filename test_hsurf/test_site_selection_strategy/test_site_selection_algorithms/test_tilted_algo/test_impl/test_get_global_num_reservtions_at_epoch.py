...
def test_get_global_num_reservations_at_epoch8():
    surface_size = 8
    for rank in range(2**8):
        epoch = get_global_epoch(rank, surface_size)
        assert get_global_num_reservations_at_epoch(
            epoch, surface_size
        ) == _expected8(
            rank,
        )
...
def test_get_global_num_reservations_at_epoch16():
    surface_size = 16
...
def test_get_global_num_reservations_at_epoch32():
    surface_size = 32
    for rank in range(2**16):
        epoch = get_global_epoch(rank, surface_size)
        assert get_global_num_reservations_at_epoch(
...
def test_get_global_num_reservations_at_epoch64():
    surface_size = 64
    for rank in range(2**16):
        epoch = get_global_epoch(rank, surface_size)
        assert get_global_num_reservations_at_epoch(
