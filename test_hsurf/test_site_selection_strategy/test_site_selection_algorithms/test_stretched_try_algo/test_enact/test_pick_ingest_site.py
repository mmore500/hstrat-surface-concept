...
def test_pick_ingest_site8():
    surface_size = 8
    for rank in range(len(expected8)):
        assert pick_ingest_site(rank, surface_size) == expected8[rank]
...
def test_pick_ingest_site16():
    surface_size = 16
    for rank in range(len(expected16)):
        assert pick_ingest_site(rank, surface_size) == expected16[rank]
...
def test_layering(surface_size: int, num_generations: int):
    df = make_surface_history_df(
        pick_ingest_site, surface_size, num_generations
    )
...
def test_pick_ingest_site_regression(
    ...
    #         ),
    #     ]:
    #         result = pick_ingest_site(rank, surface_size)
    #         print(rank, surface_size, result)
    assert expected_result == pick_ingest_site(rank, surface_size)
