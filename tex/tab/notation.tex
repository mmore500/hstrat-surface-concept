...
Current Time & int & $\colorT$ & num elapsed data item ingests & $\in$ \textsuperscript{\textdagger}$\mathbb{N}$ or \textsuperscript{\textdaggerdbl}$[0 \twodots 2^{\colorS - 1})$ \\
...
Ingest Times of Stored Data Items by Site & seq & $\colorL(\colorT)$ & $\max\{\colorTbar \in [0 \twodots \colorT) : \colorK(\colorTbar) = \colork\}$ for $\colork \in [0 \twodots \colorS)$ & $\in [0 \twodots \colorT) \cup \{\nullval\}$ \\
