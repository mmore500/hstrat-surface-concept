\begin{algorithm}[H]
\caption{Tilted algorithm ingest time lookup $\colorL_{\colorS}(\colorT)$.}
...
\begin{minipage}{0.55\textwidth}
    ...
    \Function{NaiveTiltedLookup}{$\colorS, \colorT$} \Comment{Assume buffer has been filled} \\
        ...
        \State $\texttt{uint\_t} ~ ~ g_p \gets 0$ \Comment{Physical segment index at site $\colork=0$ (i.e., left-to-right position)}
        ...
        \State $\texttt{uint\_t} ~ ~ \epsilon_w \gets \Call{I}{g_p = 0}$ \Comment{Correction factor for segment size $w$}
        ...
        \State $\texttt{uint\_t} ~ ~ \colorT_i \gets 2^{\colorh'}(2g_l' + 1) - 1$ \Comment{When will current site $\colork$ overwritten by invader?}
        ...
        \State $\texttt{uint\_t} ~ ~ \epsilon_G \gets \Call{I}{\rchi_{\mkern-5mu\mathrm{A}} \vee \rchi_{\mkern-5mu\mathrm{\hat{A}}} \vee \rchi_{\mkern-5mu\mathrm{B}} \vee \rchi_{\mkern-5mu\mathrm{\hat{B}}}} \;\; \times \;\; G'$ \Comment{Correction on guessed segment count $G'$}
        ...
        \State $\texttt{uint\_t} ~ ~ G \gets G' + \epsilon_G$ \Comment{Corrected number of segments in \hv{} ring buffer $G$}
        ...
        \State $\texttt{uint\_t} ~ ~ g_l \gets \Call{ElvisOp}{\Call{I}{\rchi_{\mkern-5mu\mathrm{A}} \vee \rchi_{\mkern-5mu\mathrm{\hat{A}}}} \;\; \times \;\; (G' + g_p), \;\; g_l'}$ \Comment{Corrected logical segment index $g_l$}
        ...
        \State $\texttt{uint\_t} ~ ~ i \gets j - \Call{ModPow2}{j - g_l + G, \;\; G}$ \Comment{Hanoi value incidence resident at site $\colork$}
