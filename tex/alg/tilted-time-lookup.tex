...
\begin{minipage}{0.55\textwidth}
    ...
    \Function{NaiveTiltedLookup}{$\colorS, \colorT$} \Comment{Assume buffer has been filled} \\
        ...
        \State $\texttt{uint\_t} ~ ~ g_p \gets 0$ \Comment{Left-to-right (physical) segment index}
        ...
        \Statex \Comment{\textbf{Scenario A}: site in invaded segment, h.v. ring buffer intact}
        ...
        \Statex \Comment{\textbf{Scenario B}: site in invading segment, h.v. ring buffer intact}
        ...
        \State $\texttt{uint\_t} ~ ~ \epsilon_{\colorh} \gets \Call{I}{X_A \vee X_A'} \;\; \times \;\; (w - w_0)$ \Comment{Correction on guessed reserved h.v. $\colorh'$}
        ...
        \State \Comment{Decode what h.v. instance fell on site $\colork$\ldots}
        ...
        \State \textbf{yield} $2^{\colorh}(2i + 1) - 1$ \Comment{\ldots then decode ingest time $\colorTbar$ of assigned h.v.}
        \State \Comment{Update state for next site iterated over\ldots}
        ...
        \State $\colorh' \gets \colorh' \;\; \times \;\; \Call{I}{\colorh' \neq w}$ \Comment{Reset h.v. to zero if segment filled and starting new segment}
        \EndFor
    \EndFunction
    \end{algorithmic}
\end{minipage}
