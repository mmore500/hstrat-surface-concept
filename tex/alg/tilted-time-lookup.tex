...
\begin{minipage}{0.55\textwidth}
    ...
    \Function{NaiveTiltedLookup}{$\colorS, \colorT$} \Comment{Assume buffer has been filled} \\
        ...
        \State $\texttt{bool\_t} ~ ~  X_A \gets \colorh' - (\colort - \colort_0) > w - w_0$ \Comment{Will \hv{} ring buffer be invaded in future epoch $\in \colortau_{\mkern-5mu\scriptscriptstyle 0}$?}
        \State $\texttt{uint\_t} ~ ~ \colorT_i \gets 2^{\colorh'}(2g_l' + 1) - 1$ \Comment{When will current site $\colork$ overwritten by invader?}
        \State $\texttt{bool\_t} ~ ~ X_A' \gets (\colorh' - (\colort - \colort_0) = w - w_0) \wedge (\colorT_i \geq \colorT)$ \Comment{This site $\colork$ to be overwritten by invader during current epoch?}
        \Statex \Comment{\textbf{Scenario B}: site in invading segment, \hv{} ring buffer intact}
        \State $\texttt{bool\_t} ~ ~ X_B \gets (\colort - \colort_0 < \colorh' < w_0) \wedge (\colort < \colorS - \colors)$ \Comment{Will \hv{} ring buffer be invaded in future epoch $\in \colortau_{\mkern-5mu\scriptscriptstyle 0}$?}
        ...
        \State $\texttt{bool\_t} ~ ~ X_B' \gets (\colorh' = \colort - \colort_0) \wedge (\colort < \colorS - \colors) \wedge (\colorT_r \geq \colorT)$ \Comment{This site $\colork$ to be refilled after invasion during current epoch?}
        \Statex \texttt{/*} \textit{Note:} $\Call{I}{X_A} + \Call{I}{X_A'} + \Call{I}{X_B} + \Call{I}{X_B'} \in \{0,1\}$ \texttt{*/} \Comment{Apply corrections for complicating scenarios\ldots}
        ...
        \State $\texttt{uint\_t} ~ ~ \epsilon_{\colorT} \gets \Call{I}{X_A' \vee X_B'} \;\; \times \;\; (\colorT - \colorT_0)$ \Comment{Need to snap back to outset of current epoch $\colort$?}
        ...
        \State $\texttt{uint\_t} ~ ~ g_l \gets \Call{ElvisOp}{\Call{I}{X_A \vee X_A'} \;\; \times \;\; (G' + g_p), \;\; g_l'}$ \Comment{Corrected logical segment index $g_l$}
