\begin{algorithm}[H]
\caption{Steady algorithm site selection $\colorK_{\colorS}(\colorT)$. Supplementary Listing \ref{lst:steady_site_selection.py} provides reference Python code.}
...
\begin{minipage}{0.5\textwidth}
    ...
    \begin{algorithmic}[1]
        ...
        \State $\texttt{uint\_t} ~ ~ \colort \gets \Call{BitLength}{\colorT+1} - \colors $ \Comment{Current epoch (or negative)}
        ...
        \State $\texttt{uint\_t} ~ ~ \colork \gets 0$ \Comment{Bunch position}
        \State $\texttt{uint\_t} ~ ~ o \gets 0$ \Comment{Within-bunch offset}
        \State $\texttt{uint\_t} ~ ~ w \gets \colors$ \Comment{Segment width}
        \Else
        ...
        \State $\texttt{uint\_t} ~ ~ \colork \gets 2^{b}(\colors - b + 1) - 1$ \Comment{Bunch position}
