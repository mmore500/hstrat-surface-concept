\begin{algorithm}[H]
\caption{Steady algorithm ingest time lookup $\colorL_{\colorS}(\colorT)$.}
...
\begin{minipage}{0.5\textwidth}
    ...
    \Function{NaiveSteadyLookup}{$\colorS, \colorT$}\\
        \hspace*{\algorithmicindent} \textbf{Input:} $\colorS \in \{2^{\mathbb{N}}\},\;\; \colorT \in [\colorS - 1 \twodots)$ \Comment{Buffer size and current logical time}\\
        ...
        \State $\texttt{uint\_t} ~ ~ g^{\scriptscriptstyle\shortdownarrow}_b \gets 1$ \Comment{Countdown on segments traversed within bunch}
        \State $\texttt{bool\_t} ~ ~ b^{*} \gets \texttt{True}$ \Comment{Flag if have traversed all segments in bunch?}
        \State $\texttt{uint\_t} ~ ~ {\colork}^{\scriptscriptstyle\shortdownarrow}_g \gets \colors$ \Comment{Countdown on sites traversed within segment}
        ...
        \State $\texttt{uint\_t} ~ ~ g_p \gets 2^b -g^{\scriptscriptstyle\shortdownarrow}_b$ \Comment{Calc left-to-right index of current segment}
        \State $\texttt{uint\_t} ~ ~ \colorh_{\max} \gets \colort + w - 1$ \Comment{Max possible \hv{} in segment during current epoch $\colort$}
        \State $\texttt{uint\_t} ~ ~ \colorh' \gets \colorh_{\max} - ((\colorh_{\max} + {\colork}^{\scriptscriptstyle\shortdownarrow}_g) \bmod w)$ \Comment{Candidate hanoi value}
        \Statex
        \Statex \Comment{Decode ingest $\colorTbar$ from physical segment index $g_p$\ldots}
        \Statex \Comment{\dots which tells instance of reserved \hv{} (i.e., how many seen)\ldots}
        \State $\texttt{uint\_t} ~ ~ \colorTbar' \gets 2^{\colorh'}(2g_p + 1) - 1$ \Comment{Guess ingest time of data item at current site}
        \State $\texttt{uint\_t} ~ ~ \epsilon_{\colorh} \gets \Call{I}{\colorTbar \geq \colorT} \;\; \times \;\; w$ \Comment{Correction on \hv{} if assigned instance not yet seen (i.e., $\colorTbar \geq \colorT$)}
        \State $\texttt{uint\_t} ~ ~ \colorh \gets \colorh' - \epsilon_{\colorh}$ \Comment{Corrected true resident \hv{} at site}
        \State $\texttt{uint\_t} ~ ~ \colorTbar \gets 2^{\colorh}(2g_p + 1) - 1$ \Comment{True ingest time}
        \State \textbf{yield} $\colorTbar$
        \Statex
        \Statex \LComment{Update state for next site iterated over\ldots}
        \State $g^{\scriptscriptstyle\shortdownarrow}_b \gets g^{\scriptscriptstyle\shortdownarrow}_b - \Call{I}{{\colork}^{\scriptscriptstyle\shortdownarrow}_g  = 0}$ \Comment{Bump to next segment within bunch}
        \State $b^{*} \gets (g^{\scriptscriptstyle\shortdownarrow}_b = 0) \wedge ({\colork}^{\scriptscriptstyle\shortdownarrow}_g = 0)$ \Comment{Should bump to next bunch?}
        \State $b \gets b + \Call{I}{b^{*}}$ \Comment{Do bump to next bunch, if should}
        \State $g^{\scriptscriptstyle\shortdownarrow}_b \gets \Call{ElvisOp}{g^{\scriptscriptstyle\shortdownarrow}_b, \;\; 2^{b - 1}}$ \Comment{Set within-bunch segment countdown if bumping to next bunch}
        \EndFor
    \EndFunction
    \end{algorithmic}
\end{minipage}
