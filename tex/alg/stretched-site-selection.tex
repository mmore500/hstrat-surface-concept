...
\begin{minipage}{0.6\textwidth}
    ...
    \begin{algorithmic}[1]
        ...
        \State $\texttt{uint\_t} ~ ~ \colort \gets \max(0,\;\; \Call{BitLength}{\colorT+1} - \colors)$ \Comment{Current epoch (or negative)}
        ...
        \State $\texttt{uint\_t} ~ ~ w \gets \Call{RightShift}{S, v} \;\; \times \;\;\Call{Bool2Int}{v > 0}$ \Comment{Num bunches spaced between bunches in same nest level}
        ...
        \Statex
        \State $\texttt{uint\_t} ~ ~ \epsilon_{\colork} = \Call{Bool2Int}{b_l > 0}$  \Comment{Correction factor for zeroth bunch (i.e., bunch $r=\colors$ at site $\colork=0$)}
