\begin{algorithm}[H]
\caption{Stretched algorithm ingest time lookup $\colorL_{\colorS}(\colorT)$. Supplementary Listing \ref{lst:stretched_time_lookup.py} provides reference Python code.}
...
\State $\texttt{uint\_t} ~ ~ \colortau_{\mkern-5mu\scriptscriptstyle 1} \gets \colortau_{\mkern-5mu\scriptscriptstyle 0} + 1$ \Comment{Next meta-epoch}
        ...
        \State $\texttt{uint\_t} ~ ~ \colorTbar_k' \gets 2^{\colorh}(2i' + 1) - 1$ \Comment{Guess ingest time}
        \State $\texttt{uint\_t} ~ ~ \epsilon_{\colorh} \gets \Call{I}{\colorTbar_k' \geq \colorT} \;\; \times \;\; (w - w_0)$ \Comment{Correction factor, reserved \hv{} $\colorh$}
        \State $\texttt{uint\_t} ~ ~ \epsilon_{i} \gets \Call{I}{\colorTbar_k' \geq \colorT} \;\; \times \;\; (m_p + M - i')$ \Comment{Correction factor, \hv{} instance $i$ (i.e., num seen)}
        \Statex
        ...
        \State $\texttt{uint\_t} ~ ~ \colorTbar_k \gets 2^{\colorh}(2i + 1) - 1$ \Comment{True ingest time}
