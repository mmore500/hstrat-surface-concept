...
\State $\texttt{uint\_t} ~ ~ \colortau_{\mkern-5mu\scriptscriptstyle 1} \gets \colortau_{\mkern-5mu\scriptscriptstyle 0} + 1$ \Comment{Next meta-epoch}
        ...
        \State $\texttt{uint\_t} ~ ~ g_p \gets 0$ \Comment{Physical segment index, $\colork=0$ (i.e., left-to-right position)}
        ...
        \State $\texttt{uint\_t} ~ ~ \epsilon_w \gets \Call{I}{g_p = 0}$ \Comment{Correction factor for segment size}
        \State $\texttt{uint\_t} ~ ~ w \gets w_1 + b_l + \epsilon_w$ \Comment{Number of sites in current segment (i.e., segment size)}
        \Statex \Comment{Determine correction factors for not-yet-seen data items $\colorTbar \geq \colorT$}
        \Statex $\texttt{uint\_t} ~ ~ i' \gets \Call{RightShift}{G + g_p, \;\; b_l + 1}$ \Comment{Guess \hv{} incidence (i.e., num seen)}
        ...
        \Statex
        \Statex \Comment{Decode ingest time of assigned hanoi value \ldots}
