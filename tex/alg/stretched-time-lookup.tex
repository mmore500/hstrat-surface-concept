...
\State $\texttt{uint\_t} ~ ~ \colortau_{\mkern-5mu\scriptscriptstyle 1} \gets \colortau_{\mkern-5mu\scriptscriptstyle 0} + 1$ \Comment{Next meta-epoch}
        \State $\texttt{uint\_t} ~ ~ G \gets \Call{Elvis}{\Call{RightShift}{\colorS, \colortau_{\mkern-5mu\scriptscriptstyle 1}}, 1}$ \Comment{Number of invading segments present at current epoch}
        ...
        \State $\texttt{uint\_t} ~ ~ i' \gets \Call{RightShift}{G + g_p, b_l + 1}$ \Comment{Guess h.v. incidence (i.e., num seen)}
        \State $\texttt{uint\_t} ~ ~ \colorTbar' \gets 2^{\colorh}(2i' + 1) - 1$ \Comment{Guess ingest time}
        \State $\texttt{uint\_t} ~ ~ \epsilon_{\colorh} \gets \Call{Bool2Int}{\colorTbar' \geq \colorT} \;\; \times \;\; (w - w_0)$ \Comment{Correction factor, $\colorh$}
        \State $\texttt{uint\_t} ~ ~ \epsilon_{i} \gets \Call{Bool2Int}{\colorTbar' \geq \colorT} \;\; \times \;\; (g_p + G - i')$ \Comment{Correction factor, $i$}
        \Statex
        ...
        \State $\texttt{uint\_t} ~ ~ i \gets \epsilon_{i}$ \Comment{True h.v. incidence}
