...
% Add text
\node[anchor=west] (texta) at (0, 3.5) {$r=1$};
% Left side arrows
...
% Add text
\node[anchor=west] (textb) at (0, 2.5) {$r=2$};
...
\node[anchor=west] (textc) at (0, 1.5) {$r=3$};
\filldraw[draw=white,ultra thick,fill=black!25] (14,5) -- ++(2.15,-3) -- ++(2.15,3) -- cycle;
\node[anchor=center] (textca) at (16, 1.5) {Seg.{} 0};

\draw[-stealth, thick,draw=MidnightBlue!40]
(textc.east) -- (textca.west);

\node[anchor=west] (textd) at (0, 0.5) {$r=4$};
\filldraw[draw=white,ultra thick,fill=black!10] (1.5,5) -- ++(3.25,-4) -- ++(3.25,4) -- cycle;
\node[anchor=center] (textda) at (5, 0.5) {Seg.{} 0};

\draw[-stealth, thick,draw=MidnightBlue!20]
(textd.east) -- (textda.west);


\node[anchor=west] (textbunch) at (-1, 4.2) {Bunch};
...
\end{subfigure}
  \caption{
    ...
    Note also how sites invaded during an epoch all share the same reserved \hv{}, causing available sites for that \hv{} to instantaneously halve.
