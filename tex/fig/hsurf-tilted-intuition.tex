...
\hfill
  ...
  \caption{
    ...
    As contrasted between left and right panels, the tilted and stretched algorithms differ in their handling of \hv{} instances after available reservation segments have been filled.
    Schematics show site selection strategy for items with \hv{} $\colorH(\colorTbar) = 0$ on a buffer of size $\colorS=8$.
    Whereas the stretched algorithm discards these items, the tilted algorithm treats reserved segments as a ring buffer by ``wrapping around'' and beginning again from largest (and leftmost) segment $r=\colors$.
