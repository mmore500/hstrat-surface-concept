...
\newcommand{\YL}{0.555} % left  panel y-scale (1 = unchanged)
    ...
    \node[anchor=north west, inner sep=0, opacity=0.0] (X) at (-0.6cm,0){%
      \includegraphics[height=3.283in,
                      ...
                      {binder-20a-tilted-eulerian-surface/binder/%
                       ...
                       swap-yaxes=True+symbol-color=black+%
                       viz=site-reservation-at-ranks-heatmap+ext=.pdf}};
      \fill[gray,fill opacity=0.32] ($(X.south west) - (-0.5cm, 0.2cm)$)
                       rectangle ($(X.south west) - (-1cm, -0.52cm)$);
      \fill[gray,fill opacity=0.18] ($(X.south west) - (-0.5cm, -1.22cm)$)
                       rectangle ($(X.south west) - (-1cm, -0.52cm)$);
      \fill[gray,fill opacity=0.055] ($(X.south west) - (-0.5cm, -1.22cm)$)
                       rectangle ($(X.south west) - (-1cm, -8.5cm)$);
...
\caption{%
  ...
  Note nested composition of consecutive-\hv ``segments'' (solid lines); buffer layout design is described in Figure \ref{fig:hsurf-stretched-intuition}.
  Section \ref{sec:restrictions} describes behavior beyond meta-epoch $\colortau=3$.
  }
