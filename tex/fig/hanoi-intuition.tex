    ...
    \begin{minipage}[c][1.30in][c]{0.90\linewidth}
      ...
      \begin{tikzpicture}[>=Stealth,decoration={brace}]
        ...
        \draw[line width=1.4pt,decorate,decoration={brace,amplitude=4pt}] ($(imgA.east)+(1.7,1.30)$) -- ($(imgA.east)+(1.7,-0.90)$)
          node[midway,anchor=west,xshift=0.34cm,align=center] {\Large Keep up to $4\times$ most recent items of each \hv{}\\(If $<4$ items of \hv{} seen, keep all \hv{} items.)};
      \end{tikzpicture}
      \hspace*{\fill}%
    \end{minipage}}\\[-1ex]
    ...
    \begin{minipage}[c][1.48in][c]{0.90\linewidth}
      ...
      \begin{tikzpicture}[>=Stealth,decoration={brace}]
        ...
        \draw[line width=1.4pt,decorate,decoration={brace,amplitude=4pt}] ($(imgB.east)+(1.7,0.90)$) -- ($(imgB.east)+(1.7,-0.90)$)
          node[midway,anchor=west,xshift=0.34cm,align=center] {\Large Keep up to $8\times$ most recent items of each \hv{}\\(If $<8$ items of \hv{} seen, keep all \hv{} items.)};
      \end{tikzpicture}
      \vspace{-0.7cm}
      \hspace*{\fill}%
    \end{minipage}}
...
\begin{subfigure}{\linewidth}
  ...
  \begin{tikzpicture}
    ...
    \begin{scope}[x={(X.south east)},y={(X.north west)}]
      ...
      \Large Keep last $1\times$ seen item of each \hv{}
      };
      ...
      \node[text width=1.5in] at ($(X.south west) + (4.2cm, 2.1cm)$) {%
      \Large Keep up to $2\times$ most recent items of each \hv{} \small (Keep all \hv{} items, if fewer.)
      };
...
\caption{\footnotesize time course of buffer layout}
...
% \vspace{-0.1in}
\caption{%
  \textbf{Layout strategy for tilted-generalized ring buffer.}
  \footnotesize
  ...
  Lower panel shows epoch-by-epoch physical storage layout for \hv sets.
  ...
  Note nested composition of consecutive-\hv ``segments;'' design is described in Figure \ref{fig:hsurf-stretched-intuition}.
  }
