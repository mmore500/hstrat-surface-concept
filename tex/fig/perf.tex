...
\centering
\includegraphics[width=0.92\textwidth]{binder-downstream-benchmark/binder/teeplots/2025-01-18-cpp-bench-speed/color=black+exclude=dstream.stretched_algo,dstream.tilted_algo,zhao_tilted_algo+palette=tab10-r+row=is-naive+viz=outsetgrid+x=num-sites+y=duration-per-item-ns+ext=.png}
...
Over 1M ingest trials, the dstream approach provided between $2.5\times$ and $72\times$ speedup over the naive approach, shown in subpanel \ref{fig:perf:speed}.
Detailed timing results are proved in Table \ref{tab:perf}.
Per-item naive and dstream walltime increases for shorter 10k ingest trials, because a higher fraction of items are stored rather than discarded.
Timings for maintaining a simple ring buffer and no-store processing are included as controls.
For longer 1M ingest trials, dstream per-item walltime is nearly equivalent to no-store control.
However, for shoter 10k ingest trials, dstream performance more closely resembles that of the ring buffer control.
