...
As a simplifying assumption, we have reduced data ingestion to a sole update operation: ``site selection,'' picking a buffer indices for the $n$th received data item --- overwriting any existing data item at that location.
...
Interpereting stored data, therefore, additionally requires support for ``inverse'' decoding of ingest time based solely on an item's buffer index $\colork$ and current time $\colorT$.
...
One possible approach to this issue would simply be designation of a canonical implementation and development of language-agnostic tests to validate other implementations.
