...
Formally, we frame our objectives in terms of cost functions on the timepoints of discarded data items, formulated in terms of
\begin{itemize}
\item logical time $\colorT$, which reflects how many data items have been ingested),
\item timepoints $\colorTbar$ of ingested data items, and
\item ``gap size'' $\colorG_{\colorT}(\colorTbar)$ in the curated record, the number of discarded data items around timepoint $\colorTbar$ at logical time $\colorT$.
\end{itemize}
