...
\begin{empheq}[left={\hspace{1.5in}\displaystyle \mathsf{cost}(\colorT) = \max_{\colorTbar \in [0\twodots\colorT)} \empheqlbrace}]{align}
  &\colorG_{\colorT}(\colorTbar) &&\text{for \textit{``steady''} curation,}  &&& ~ &&& ~ \label{eqn:steady-cost} \\
  &\frac{\colorG_{\colorT}(\colorTbar)}{\max(\colorTbar,\;\; 1)} &&\text{for \textit{``stretched''} curation, and}  &&& ~ &&& ~ \label{eqn:stretched-cost} \\
  &\frac{\colorG_{\colorT}(\colorTbar)}{\colorT - \colorTbar} &&\text{for \textit{``tilted''} curation;}  &&& ~ &&& ~ \label{eqn:tilted-cost}
\end{empheq}
...
Formally, our objective is to maintain $\mathsf{cost}(\colorT)$ below an upper bound $\mathsf{bound}(\colorT)$, specified on a per-algorithm basis, across logical time $\colorT$.
