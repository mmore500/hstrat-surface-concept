...
By Equation \ref{eqn:stretched-best}, the tilted curation algorithm guarantees gap size ratio within a factor of $(1 + 1/S)\min(2t + 2s, 4t, 2^{\colortau + 1})$ times optimal.
Because $\min(2\colort + 2\colors,\;\; 4\colort,\;\; 2^{\colors - \colortau - 1}) \leq \colorS$, we have from Equation \ref{eqn:stretched-best} we that the tilted curation algorithm guarantees gap size ratio within a factor of $2(1 + 1/S)\min(2t + 2s, \;\; 4t, \;\; 2^{\colortau + 1})$ times optimal.
