...
\begin{align*}
\max\{\frac{\colorG_{\colorT}(\colorTbar)}{\colorT - 1 - \colorTbar} : \colorTbar \in [0 \twodots \colorT-1).
\end{align*}
...
Additionally, gap size ratio is bounded $\frac{\colorG_{\colorT}(\colorTbar)}{\colorT - 1 - \colorTbar} \leq 2$.
...
In this final subsection, we establish an upper bound on gap size ratio $\colorg/(\colorT - 1 - \colorTbar)$ for a buffer of size $\colorS$ at time $\colorT$ under the proposed tilted curation algorithm.

\input{thm/tilted-gap-size}

During early epoch $\colort = 1$, $\colorG_{\colorT}(\colorTbar)/(\colorT - 1 - \colorTbar) \leq 4/\colorS$.
Likewise, during the last supported meta-epoch $\colortau = \colors - 1$, $\colorG_{\colorT}(\colorTbar)/(\colorT - 1 - \colorTbar) \leq 2$.
