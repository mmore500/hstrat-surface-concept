...
Note that if no data item has yet been stored at a stie (i.e., when first filling the buffer $\colorT < \colorS$), $\colorL(\colorT)$ may include $\nullval$ values.%
\footnote{%
As with site selection $\colorK$, although omitted for brevity it is the case that lookup depends on buffer size (i.e., as $\colorL_{\colorS}(\colorT)$).
}
...
Proposed algorithms make heavy use of OEIS integer sequence A007814 \citep{oeis}, forulated as
\begin{align}
\colorH(\colorTbar)
\coloneq
...
For the \textit{steady algorithm}, an epochs' first time point $\colorT = \min\colortsetofT$  sits one less than an even power of two (e.g.,  $\colorT = 15$).
...
Algorithm listings refer to a handful of utility helper functions (e.g., \Call{BitCount}, \Call{BitLength}, etc.).
