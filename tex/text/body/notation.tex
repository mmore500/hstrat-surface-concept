...
We refer to $\colorH(\colorT) = \colorh$ as the ``\textbf{hanoi value}'' of $\colorT$ in reference to the famous ``tower of hanoi'' puzzle \citep{lucas1889jeux}.
...
We call this value $\colort \sim \left\lfloor \log_2(\colorT) \right\rfloor$ the \textbf{epoch} of $\colorT$, correcting to begin epoch 1 at $\colorT = \colorS$ and taking $\colorT < \colorS$ (before the buffer fills) as epoch 0.
