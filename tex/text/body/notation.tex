...
For the \textit{steady algorithm}, epoch transition occur immediately \textit{before} each binary order of magnitude $\colorT \geq \colorS - 1$ (e.g.,  $\colorT = 15$).
For the \textit{stretched} and \textit{tilted algorithms}, epoch transition instead occurs exactly at each binary order of magnitude (e.g., $\colorT = 16$) for $\colorT \geq \colorS$.
Formally,
\begin{align*}
\colort &= \max\Big(\left\lfloor \log_2(\colorT+1) \right\rfloor - \colors + 1, 0\Big) \tag{steady criterion algorithm}\\
&\text{or}\\
\colort &= \max\Big(\left\lfloor \log_2(\colorT - 1) \right\rfloor - \colors + 1, 0\Big) \tag{stretched/tilted criterion algorithms}.
\end{align*}
