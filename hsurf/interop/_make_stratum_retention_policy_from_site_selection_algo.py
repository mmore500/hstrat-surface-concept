...
def make_stratum_retention_policy_from_site_selection_algo(
    ...
    class CalcNumStrataRetainedExactFtor(_CalcNumStrataRetainedExactFtorBase):
        ...
        def __call__(
            ...
            return sum(
                1 for __ in policy.IterRetainedRanks(num_strata_ingested)
            )
    ...
    class CalcRankAtColumnIndexFtor(_CalcRankAtColumnIndexFtorBase):
        ...
        def __call__(
            ...
            if (
                not 0
                <= index
                < policy.CalcNumStrataRetainedExact(num_strata_ingested)
            ):
                raise IndexError
            res = mit.nth(policy.IterRetainedRanks(num_strata_ingested), index)
    ...
    class GenDropRanksFtor(_GenDropRanksFtorBase):
        ...
        def __call__(
            self: "GenDropRanksFtor",
            policy: PolicyCouplerBase,
            num_stratum_ingests_completed: int,
            retained_ranks: typing.Optional[typing.Iterable[int]] = None,
        ) -> typing.Iterator[int]:
            # convert for compat with numpy dtypes
            assert isinstance(
                num_stratum_ingests_completed, numbers.Integral
            )
            num_stratum_ingests_completed = int(
                num_stratum_ingests_completed,
            )

            if num_stratum_ingests_completed == 0:
                return
            ...
            target_site = algo.pick_ingest_site(
                num_stratum_ingests_completed, surface_size
            )
            target_rank = algo.calc_resident_ingest_rank(
                target_site, surface_size, num_stratum_ingests_completed
            )
            if target_rank != 0:
                yield target_rank
                return
                ...
                surface_size,
                num_stratum_ingests_completed + 1,
            ):
                yield 0
                return
    ...
    class IterRetainedRanksFtor(_IterRetainedRanksFtorBase):
        ...
        def __call__(
            ...
            # convert for compat with numpy dtypes
            assert isinstance(num_strata_ingested, numbers.Integral)
            num_strata_ingested = int(num_strata_ingested)

            if num_strata_ingested == 0:
                return
            algo = site_selection_algo
            surface_size = policy.GetSpec().GetSurfaceSize()
            ranks = sorted(
