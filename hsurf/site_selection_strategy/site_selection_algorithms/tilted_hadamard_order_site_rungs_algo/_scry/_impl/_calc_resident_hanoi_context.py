...
def calc_resident_hanoi_context(
    ...
    hanoi value associated with the ingested stratum at site `site`?

    Returns None if the resident stratum traces back to original randomization
    of the surface prior to any algorithm-determined stratum ingests.
    """

    # special case for zero ingests,
    ...
    # 2. there have actually been enough ingests of the candidate hanoi
    #    value to reach the site's semantic index within the incidence
    #    reservation buffer at least once
    # if at least one of these conditions is not fulfilled, then no ingest
    ...
    # have ingested there (it would still be around because it would have
    ...
    for candidate_hanoi_value, candidate_reservation_index in candidate_zip:
        # focal rank: the very last possible time when a ingest of
        ...
        if (
            ...
            # could have possibly been ingested at site (because at this point
            ...
            if deadline_rank is None:
                ...
                # incidence)
                # thus, candidate hanoi value could never have ingested at
        ...
        # how many ingests of candidate_hanoi_value have been performed
        ...
        # have enough ingests of candidate_hanoi_value value been performed
        # to fill up candidate_hanoi_value's incidence reservation buffer
        # sufficiently to reach site and put a value there?
        # if this is the case, then the resident ingest at site is
        ...
        if hanoi_count > candidate_reservation_index:
            ...
            assert 0 <= focal_rank < num_ingests
            return {
        ...
        # candidate_hanoi_value did not have enough ingests to make it to
        # site, so whatever hanoi value site was allocated to beforehand would
        # still be the resident ingest (if it succeeded in reaching the
        # corresponding incidence reservation buffer position of site)
        # ... so, continue with loop to look at the next candidate

    # fall-through case: the site has not been ingested on, so the site holds
