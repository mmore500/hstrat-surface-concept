...
def _make_do_init(
    ...
    def do_init() -> typing.Sequence[mpl_artist.Artist]:
        ...
        buffer_ax.set_xticklabels([])
        reservation_ax.set_yticks([])
        n_site = surface_history_df["site"].max() + 1
        _draw_buffer_grid(buffer_ax, n_site)
        _draw_reservation_grid(reservation_ax, n_site)
        fig.tight_layout()
        ...
        age_patches = extra_patches[n_site : n_site * 2]
...
def typewriter_with_reservations_animate(
    ...
    reservation_ax = fig.add_subplot(gs[2, 0])
